...
export const useCountryData = () => {
    ...
    const getImageUrl = async (countryId: string): Promise<string> => {
        ...
        if (apiKey) {
            try {
                ...
                const response = await fetch(`https://api.unsplash.com/search/photos?query=${encodeURIComponent(countryName)}&orientation=landscape&per_page=1`, {
                    headers: {
    ...
    const getCountrySuggestions = (input: string): typeof allCountries => {
        ...
        return allCountries.filter(c =>
            ...
            (c.japaneseName && c.japaneseName.toLowerCase().startsWith(normalizedInput))
        ).slice(0, 5); // Return top 5 matches
    };

    return { getImageUrl, getCountryName, findCountryA3CodeByName, getCountrySuggestions };
};
